...
import { FilePen, Clock3, Building2, Headphones, Award, GraduationCap } from 'lucide-react';
...
const CollegeProcess = () => {
  ...
  const steps = [
    ...
    { id: 6, label: 'Admission Confirmation', icon: <Award />, completed: false }
  ];

  const features = [
    {
      title: 'Get Your Career Match',
      description: 'Take our free career compass personality quiz and get top career options for you.',
    ...
    },
    {
      ...
      description: 'Apply to and manage all college applications through My Profile',
      icon: <Building2 className="w-8 h-8" />,
      color: 'bg-green-500'
    },
    {
      title: 'Expert Guidance',
      description: 'Get free personalised expert guidance on colleges & courses',
                ...
                <div className={`flex flex-col items-center ${index !== steps.length - 1 ? 'w-full' : ''}`}>
                  <div 
                    className={`w-16 h-16 rounded-full flex items-center justify-center transition-all duration-700 transform hover:scale-110 ${
                      index === activeStep ? 'bg-blue-600 scale-110' :
                  ...
                  <span className={`text-sm mt-3 font-medium transition-all duration-500 ${
                    index === activeStep ? 'text-blue-600 scale-110' : 'text-gray-600'
                  }`}>
                    {step.label}
                  </span>
                </div>
        ...
        {features.map((feature, index) => (
          <div
            ...
            className={`transform transition-all duration-1000 ${
              isVisible[index] ? 'translate-y-0 opacity-100' : 'translate-y-16 opacity-0'
            }`}
            ...
            <div className="bg-white rounded-xl shadow-lg overflow-hidden group hover:shadow-xl transition-all duration-300">
              <div className="p-6">
                <div className="flex items-start space-x-4">
                  <div className={`${feature.color} p-4 rounded-xl text-white transform transition-all duration-500 group-hover:rotate-12 group-hover:scale-110`}>
                    {feature.icon}
                  </div>
